...
/**
 * الجهة في CRM = جهة من دليل «جهات الاتصال» (contacts_organizations) + علاقة
 * بمساحة (crm_workspace_orgs) تحمل تصنيفها ومسؤولها وحالتها ووسومها.
 *
 * الدليل يملك هوية الجهة وبياناتها، وCRM يملك العلاقة وحدها. فالجهة تبقى في
 * الشركة وإن عُطّل CRM، وتحديث هاتفها من أي مكان يصحّحه في كل مكان.
 */
class Organization
{
    public static function find(int $id): ?array
    {
        return Directory::findOrg($id);
    }

    public static function create(array $data): int
    {
        return Directory::createOrg($data);
    }

    public static function update(int $id, array $data): void
    {
        Directory::updateOrg($id, $data);
    }
    ...
    {
        return Directory::organizations($companyId, ['q' => $q], $limit);
    }
    ...
    public static function possibleDuplicates(int $companyId, string $name, ?int $excludeId = null): array
    {
        ...
        $sql = 'SELECT id, name, city FROM contacts_organizations WHERE company_id = :c AND name LIKE :q';
    ...
    /** ربط جهة من الدليل بمساحة (أو إرجاع الربط الموجود). */
    ...
    public static function inWorkspace(int $workspaceId, array $filters = [], int $limit = 50, int $offset = 0): array
    {
        ...
        return Database::select(
            "SELECT r.*, o.name, o.trade_name, o.sector, o.city, o.country, o.email, o.phone, o.logo, o.kind,
    ...
    public static function countInWorkspace(int $workspaceId, array $filters = []): int
    {
        ...
        return (int) (Database::first(
            "SELECT COUNT(*) AS c FROM crm_workspace_orgs r JOIN contacts_organizations o ON o.id = r.organization_id WHERE {$where}",
            $params
        )['c'] ?? 0);
    }
    ...
    public static function addTag(int $workspaceId, int $relationId, string $name): void
    {
        ...
        $tag = Database::first('SELECT id FROM crm_tags WHERE workspace_id = :w AND name = :n', ['w' => $workspaceId, 'n' => $name]);
        $tagId = $tag['id'] ?? Database::insert('crm_tags', [
            'workspace_id' => $workspaceId,
            'name' => $name,
            'created_at' => date('Y-m-d H:i:s'),
        ]);
        $linked = Database::first('SELECT tag_id FROM crm_org_tags WHERE workspace_org_id = :r AND tag_id = :t', ['r' => $relationId, 't' => $tagId]);
        if (!$linked) {
